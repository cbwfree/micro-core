...
func TodayTime() time.Time {
	return time.Unix(StrToUnix(fmt.Sprintf("%s 00:00:00", FormatDate(time.Now()))), 0)
}
...
// DateToUnix 将日期字符串转为时间戳
func DateToUnix(date string) int64 {
	date = strings.TrimSpace(date)
	s := StrToUnix(date, "Y-m-d")
	if s == 0 {
		s = StrToUnix(date, "Ymd")
	}
	if s == 0 {
		s = StrToUnix(date, "Y-n-j")
	}
	if s == 0 {
		s = StrToUnix(date, "Y/m/d")
	}
...
// DateToUnix 将日期时间字符串转为时间戳
func DateTimeToUnix(datetime string) int64 {
	datetime = strings.TrimSpace(datetime)
	s := StrToUnix(datetime, "Y-m-d H:i:s")
	if s == 0 {
		s = StrToUnix(datetime, "Y-n-j H:i:s")
	}
	if s == 0 {
		s = StrToUnix(datetime, "Y/m/d H:i:s")
	}
